...
In this step you'll add a source catalog. You won't use its servers just yet — one of them, `customer-demo-data`, is used in the next step to demonstrate data filtering.
...
Obot syncs the repository and adds its servers to the catalog. Browse the catalog and confirm you can see the servers defined by `academy-catalog` — including **`customer-demo-data`**, which you'll come back to in the next step.
