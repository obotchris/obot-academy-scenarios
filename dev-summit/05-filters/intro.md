...
In this final block you'll first add a **source catalog** — a Git repository that defines additional MCP servers your organisation wants to make available. That catalog includes a `customer-demo-data` server that returns fake records (names, email addresses, and US driving licence numbers) — safe test data for exercising filters. You'll then create gateway filters against it and see why **testing** a filter matters.
