...
In Block 3 you saw that the GitHub MCP server ran through the **Obot gateway**, so every call was logged centrally. But developers also run MCP servers **locally** — configured straight into their AI client, running on their own machine, never touching the gateway. Normally that activity is a blind spot for the organisation.
...
Alongside the gateway entries from Block 3, you'll now see entries for the **local** MCP server's tool calls — forwarded from your machine by `obot-sentry`. Because these originated locally rather than through the gateway, they're attributed to the device and user that `obot-sentry` reported them from.
